...
export default createRoute(async (c) => {
  ...
  const category = await c.env.DB.prepare(`SELECT id, name, cover_url, slug FROM categories WHERE slug = ?`).bind(slug).first()
  if (!category) return c.notFound()

  // 2. Ambil Produk Publik Saja (is_visible = 1, status = active)
  ...
  // Pengecekan apakah ini kategori voucher (untuk peringatan keamanan SN)
  const isVoucher = slug.includes('voucher') || category.name.toLowerCase().includes('voucher')
    ...
    <div class="max-w-5xl mx-auto space-y-6 pb-12">
      ...
      <div class="relative rounded-3xl overflow-hidden bg-slate-900 aspect-[4/1] md:aspect-[6/1] shadow-xl mt-4">
         ...
         <div class="absolute bottom-0 left-0 p-6 md:p-8">
           ...
           <p class="text-slate-300 mt-2 text-sm md:text-base">Checkout instan. Tanpa daftar, langsung proses!</p>
         </div>
      </div>

      {/* ======================================================== */}
      {/* AREA CHECKOUT (DIBUNGKUS ID UNTUK DIMANIPULASI AJAX) */}
      {/* ======================================================== */}
      <div id="checkoutArea" class="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div class="md:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200 p-6 rounded-2xl shadow-sm">
               ...
               {/* Input Nominal Bebas (Hidden by default) */}
      ...
      {/* ======================================================== */}
      {/* AREA QRIS & STATUS (MUNCUL SETELAH CHECKOUT AJAX SUKSES) */}
      {/* ======================================================== */}
      <div id="qrisArea" class="hidden flex-col items-center justify-center bg-white border border-slate-200 p-6 md:p-10 rounded-3xl shadow-xl max-w-lg mx-auto mt-8">
         ...
         {/* AREA VOUCHER RAHASIA (MUNCUL JIKA SUKSES & MENDAPATKAN SN) */}
      ...
      {/* ======================================================== */}
      {/* SCRIPT AJAX & UI LOGIC */}
      {/* ======================================================== */}
      <script dangerouslySetInnerHTML={{ __html: `
        // 1. LOGIKA PERUBAHAN FORM
        ...
        function updateSummary() {
          ...
          if (selectedProduct) {
            ...
            // Buka gembok tombol bayar jika nomor tujuan sudah diisi minimal 4 angka & harga > 0
        ...
        // 2. LOGIKA CHECKOUT KE BACKEND (AJAX)
        ...
        btnCheckout.addEventListener('click', async () => {
           ...
           try {
             // Tembak Public API yang kita buat sebelumnya
             ...
             if (data.success) {
                ...
                // Jalankan Timer & Radar Pengecek
                startCountdown(15 * 60);
                startPolling(data.order_id, phone);

                // Otomatis Scroll ke area QRIS
        ...
        // 4. FUNGSI RADAR PENGECEK (POLLING) SETIAP 3 DETIK
        function startPolling(orderId, phone) {
          pollInterval = setInterval(async () => {
            try {
              // Tembak API Track Order yang membutuhkan No HP (untuk keamanan)
              ...
              if (json.success) {
                ...
                if (st === 'success') {
                   ...
                   // Tampilkan Voucher Rahasia (SN) Jika Tersedia
                   if (json.data.sn) {
              ...
              // Abaikan error jaringan sementara, biarkan radar jalan terus
            }
          }, 3000);
        }
        
        // Helper sederhana pengganti URLSearchParams yang bermasalah di beberapa browser lama
        function newSearchParams(obj) {
            return Object.keys(obj).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(obj[k])).join('&');
        }
      `}} />
    </div>,
    { title: `Beli ${category.name} - Checkout Instan` }
  )
})

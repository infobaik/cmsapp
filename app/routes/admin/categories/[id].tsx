        ...
        <form method="POST" action={`/api/admin/v1/categories/${id}/update`} enctype="multipart/form-data" class="space-y-4">
          ...
          {category.image_url && (
            <div class="mb-4">
              ...
              <img src={category.image_url as string} alt="Current Icon" class="w-16 h-16 rounded-xl border border-slate-700 object-cover" />
          ...
          <div>
            ...
            <input type="file" name="icon" accept="image/*" class="w-full bg-[#121217] border border-slate-800/60 rounded-xl p-2 text-slate-200 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-600/20 file:text-blue-400 hover:file:bg-blue-600 hover:file:text-white transition-all" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">Nama Kategori</label>
            <input type="text" name="name" value={category.name} required class="w-full bg-[#121217] border border-slate-800/60 focus:border-blue-500/50 rounded-xl p-3 text-slate-200 outline-none" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 mb-1.5">Slug URL</label>
            <input type="text" name="slug" value={category.slug} required class="w-full bg-[#121217] border border-slate-800/60 focus:border-blue-500/50 rounded-xl p-3 text-slate-400 text-sm outline-none" />
          </div>

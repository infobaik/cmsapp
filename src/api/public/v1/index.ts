...
app.get('/kategori/:id', async (c) => {
  try {
    ...
    const category = await c.env.DB.prepare(`SELECT * FROM categories WHERE id = ?`).bind(strId).first() ||
                     await c.env.DB.prepare(`SELECT * FROM categories WHERE id = ?`).bind(intId).first()
    ...
    let subCategories = await c.env.DB.prepare(`SELECT id, name, slug, image_url, cover_url FROM categories WHERE parent_id = ? ORDER BY name ASC`).bind(strId).all().then(r => r.results) || []
    if (subCategories.length === 0) {
      subCategories = await c.env.DB.prepare(`SELECT id, name, slug, image_url, cover_url FROM categories WHERE parent_id = ? ORDER BY name ASC`).bind(intId).all().then(r => r.results) || []
    }
    ...
    if (subCategories.length === 0) {
      ...
      products = await c.env.DB.prepare(qStr).bind(strId).all().then(r => r.results) || []
      
      if (products.length === 0) {
        products = await c.env.DB.prepare(qStr).bind(intId).all().then(r => r.results) || []
      }
    }
